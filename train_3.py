...
import torch
from re_hifuse import HiFuse_Tiny
import numpy as np
...
def whole_face_block_coordinates():
    df = pandas.read_csv(r'D:\tengxunsp\CloFormer-main\dataset\data\442.csv')
    m, n = df.shape
    base_data_src = r'D:\tengxunsp\CloFormer-main\dataset\data\apex'
    total_emotion = 0
    image_size_u_v = 224
    # 存储图像路径的字典
...
def crop_optical_flow_block():
    # 其他部分保持不变
    whole_optical_flow_path = r'D:\tengxunsp\CloFormer-main\dataset\data\flow'
...
def main(config):
    ...
    if (config.train):
        if not path.exists('ourmodel_threedatasets_weights_bestm'):
            os.mkdir('ourmodel_threedatasets_weights_bestm')#检查是否存在保存模型权重的文件夹，如果不存在则创建。
    ...
    main_path = r'D:\tengxunsp\CloFormer-main\dataset\data\loso'
    ...
    for n_subName in subName:#遍历主数据集下的每个子文件夹
        ...
        weight_path = 'ourmodel_threedatasets_weights_bestm' + '/' + n_subName + '.pth'
        ...
        for epoch in range(1, epochs + 1):#对每个epoch进行迭代。
            ...
            if best_accuracy_for_each_subject <= val_acc:#它检查当前的验证准确率是否大于等于之前记录的最佳准确率。
                ...
                if val_acc == 1.0:  # 如果测试准确率为100%
                    directory = 'ourmodel_threedatasets_weights_bestm'
                    if not os.path.exists(directory):
                        os.makedirs(directory)  # 创建目录
                    weight_path_subject = f'ourmodel_threedatasets_weights_bestm/{n_subName}_best_accuracy.pth'

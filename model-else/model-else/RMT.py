...
class SwishImplementation(torch.autograd.Function):#自定义的 Swish 激活函数的实现
    @staticmethod
    def forward(ctx, i):
...
class MemoryEfficientSwish(nn.Module):#用于高效地应用 Swish 激活函数
    def forward(self, x):
        return SwishImplementation.apply(x)


def rotate_every_two(x):#函数将输入张量的最后一个维度按每两个元素进行旋转操作
    x1 = x[:, :, :, :, ::2]
    x2 = x[:, :, :, :, 1::2]
...
def theta_shift(x, sin, cos):#theta_shift 函数基于正弦和余弦值对输入张量进行变换
    return (x * cos) + (rotate_every_two(x) * sin)


class DWConv2d(nn.Module):#实现了一个深度可分离卷积层
...
class RetNetRelPos2d(nn.Module):#实现了一种相对位置嵌入机制，用于二维空间上相邻位置的关系建模
    ...
    def generate_2d_decay(self, H: int, W: int):#生成一个二维衰减掩码，用于在二维空间（高 H 和宽 W）上生成每对位置间的相对衰减因子。
        '''
    ...
    def generate_1d_decay(self, l: int):#生成一维衰减掩码，用于处理单维序列上的位置关系
        '''
    ...
    def forward(self, slen: Tuple[int], activate_recurrent=False, chunkwise_recurrent=False):
        '''#activate_recurrent：如果为 True，启用全局递归计算。chunkwise_recurrent：如果为 True，启用块状递归计算。
        slen: (h, w)
        h * w == l
        recurrent is not implemented
        '''
        if activate_recurrent:#ctivate_recurrent 为 True 的情况
            sin = torch.sin(self.angle * (slen[0] * slen[1] - 1))#使用正弦和余弦函数生成单一位置编码对 (sin, cos)。
            cos = torch.cos(self.angle * (slen[0] * slen[1] - 1))
            retention_rel_pos = ((sin, cos), self.decay.exp())#包括 (sin, cos) 编码和指数衰减因子。
...
@register_model
def RMT_T3(args=None, num_classes=3):
    model = VisRetNet(

...
# 读取 Excel 文件
df = pd.read_excel(r'D:\tengxunsp\CloFormer-main\dataset\data\442.xlsx')

# 确保 'flowimg' 列的数据是字符串类型，并处理缺失值
df['flowimg'] = df['flowimg'].fillna('').astype(str).str.strip()
...
# 指定原始图片文件夹路径
original_images_dir = r'D:\tengxunsp\CloFormer-main\TVL1'

# 指定新数据集的文件夹路径
base_dir = r'D:\tengxunsp\CloFormer-main\更改后数据集-平滑\442loso_li'
...
for subject in df['subject'].unique():
    ...
    for _, row in subject_df.iterrows():
        ...
        # 直接使用 flowimg 列中的文件名来查找图片
        src_file = os.path.join(original_images_dir, img_file)

...
def apply_random_mask(images, mask_ratio=0.3):
    ...
    mask = torch.rand(B, 1, H, W, device=images.device) > mask_ratio
    mask = mask.float()
...
class main_model(nn.Module):
    ...
    def __init__(self, num_classes, patch_size=4, in_chans=3, embed_dim=96, depths=(2, 2, 2, 2),
                 ...
                 use_checkpoint=False, HFF_dp=0.,#Hierarchical Feature Fusion的Dropout概率。
                 conv_depths=(2, 2, 2, 2), conv_dims=(96, 192, 384, 768), conv_drop_path_rate=0.,#卷积分支的相关参数。
                 conv_head_init_scale: float = 1., **kwargs):#卷积头初始化的缩放因子。
        super().__init__()

        ###### Local Branch Setting #######

        self.downsample_layers = nn.ModuleList()   # stem + 3 stage downsample定义局部分支的下采样层，包括一个stem层和三个下采样阶段
        stem = nn.Sequential(nn.Conv2d(in_chans, conv_dims[0], kernel_size=4, stride=4),
                             LayerNorm(conv_dims[0], eps=1e-6, data_format="channels_first"))#使用Conv2d进行初步下采样（卷积核大小4，步幅4），然后应用LayerNorm。
        ...
        self.pos_drop = nn.Dropout(p=drop_rate)#位置编码后的Dropout层。

        # stochastic depth
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))]  # stochastic depth decay rule计算每个层的Drop Path率，线性递增以实现衰减规则。
        ...
        ###### Hierachical Feature Fusion Block Setting #######  递归地初始化模型的权重。
    ...
    def _init_weights(self, m):
        if isinstance(m, nn.Linear):
            nn.init.trunc_normal_(m.weight, std=.02)#对于nn.Linear层，使用截断正态分布初始化权重，偏置初始化为0。
            if isinstance(m, nn.Linear) and m.bias is not None:
                nn.init.constant_(m.bias, 0)
        elif isinstance(m, nn.LayerNorm):#对于nn.LayerNorm层，偏置初始化为0，权重初始化为1。
            nn.init.constant_(m.bias, 0)
            nn.init.constant_(m.weight, 1.0)
        elif isinstance(m, (nn.Conv2d, nn.Linear)):#对于nn.Conv2d和nn.Linear层（冗余检查），使用截断正态分布初始化权重，偏置初始化为0。
...
#l: 局部分支特征 (Local)  l: 局部分支特征 (Local)   f: 上一级融合特征 (Feature from previous HFF_block)
        W_local = self.W_l(l)   # local feature from Local Feature Block
        W_global = self.W_g(g)   # global feature from Global Feature Block
        if f is not None:## 上一级融合特征存在时的处理
            W_f = self.Updim(f)# 上一级融合特征上采样
            W_f = self.Avg(W_f) # 平均池化
            shortcut = W_f# 残差连接
            X_f = torch.cat([W_f, W_local, W_global], 1)# 拼接特征
            X_f = self.norm1(X_f)# 归一化
            X_f = self.W3(X_f)# 卷积转换
            X_f = self.gelu(X_f)# 激活
        else:# 上一级融合特征不存在时的处理
            shortcut = 0
            X_f = torch.cat([W_local, W_global], 1)# 只拼接局部和全局特征
            X_f = self.norm2(X_f)
            X_f = self.W(X_f)
            X_f = self.gelu(X_f)

        # spatial attention for ConvNeXt branch   空间注意力机制处理卷积分支特征
        l_jump = l
        max_result, _ = torch.max(l, dim=1, keepdim=True)# 通道最大值
        avg_result = torch.mean(l, dim=1, keepdim=True) # 通道平均值
        result = torch.cat([max_result, avg_result], 1)# 拼接
        l = self.spatial(result)# 空间注意力卷积
        l = self.sigmoid(l) * l_jump # 应用注意力权重

        # channel attetion for transformer branch  通道注意力机制处理Transformer分支特征
        g_jump = g
        ...
        g = self.sigmoid(max_out+avg_out) * g_jump# 应用通道注意力权重
        # 融合特征
        fuse = torch.cat([g, l, X_f], 1)# 拼接全局、局部和融合特征
        fuse = self.norm3(fuse) # 归一化
        fuse = self.residual(fuse)# 通过IRMLP模块处理
        fuse = shortcut + self.drop_path(fuse)# 残差连接和Drop Pat
        return fuse
...
class PatchEmbed(nn.Module):#负责将输入的二维图像分割为不重叠的补丁（patches），并将每个补丁嵌入到高维空间。
    """
    ...
    def __init__(self, patch_size=4, in_c=3, embed_dim=96, norm_layer=None):
        ...
        self.in_chans = in_c
        self.embed_dim = embed_dim  #嵌入维度，即每个补丁被嵌入到的高维空间的维度。
        self.proj = nn.Conv2d(in_c, embed_dim, kernel_size=patch_size, stride=patch_size)#使用一个卷积层将输入图像分割为不重叠的补丁，并将每个补丁的通道数从 in_c 转换为 embed_dim。
        self.norm = norm_layer(embed_dim) if norm_layer else nn.Identity()

    def forward(self, x):
        _, _, H, W = x.shape  #获取输入特征的高度和宽度:

        # padding
        pad_input = (H % self.patch_size[0] != 0) or (W % self.patch_size[1] != 0)
        if pad_input:#如果输入图像的高度或宽度不能被 patch_size 整除，则在右侧和底部进行填充，使其能够被 patch_size 整除。
        ...
        x = x.flatten(2).transpose(1, 2)#flatten(2)将特征图的空间维度（H, W）展平为单一维度，即 [B, C, HW]。transpose(1, 2): 转置为 [B, HW, C]，方便后续的序列处理。
        x = self.norm(x)
        return x, H, W

class PatchMerging(nn.Module):#PatchMerging 类用于在特征图上执行下采样操作，类似于Swin Transformer中的Patch Merging层。它通过合并相邻的补丁来减小特征图的空间尺寸，同时增加特征的通道数，从而实现信息的压缩和特征的融合。
    ...
    def __init__(self, dim, norm_layer=nn.LayerNorm):
        super().__init__()
        dim = dim//2#dim: 输入特征的通道数。初始化时将其减半
        self.dim = dim
        self.reduction = nn.Linear(4 * dim, 2 * dim, bias=False)#用于将合并后的特征从 4 * dim 维度降维到 2 * dim，实现特征的融合和压缩。
        self.norm = norm_layer(4 * dim)#对合并后的特征应用归一化，以稳定特征分布。
    ...
    def forward(self, x, H, W):
        ...
        x = x.view(B, H, W, C)#将特征从 [B, H*W, C] 重塑为 [B, H, W, C]，方便后续的空间操作。
        ...
        x3 = x[:, 1::2, 1::2, :]  # [B, H/2, W/2, C]  #提取2x2区域的特征:
        x = torch.cat([x0, x1, x2, x3], -1)  # [B, H/2, W/2, 4*C] 将4个补丁的特征在通道维度上拼接，得到每个2x2区域的特征，形状为 [B, H/2, W/2, 4*C]。
        x = x.view(B, -1, 4 * C)  # [B, H/2*W/2, 4*C]  重塑为 [B, H/2*W/2, 4*C]，方便后续的线性层处理。

        x = self.norm(x)#对合并后的特征应用归一化
        x = self.reduction(x)  # [B, H/2*W/2, 2*C] 通过线性层将通道数从 4*C 降维到 2*C，同时保持空间尺寸减半。

        return x

def HiFuse_Tiny(num_classes: int):
    model = main_model(depths=(1, 1, 1, 1),
                     conv_depths=(1, 1, 1, 1),

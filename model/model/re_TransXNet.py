...
"""
class HybridTokenMixer(nn.Module):#只有 FusionConvAttention
    def __init__(self,
                 dim,
                 kernel_size=3,
                 num_groups=2,
        ...
        self.global_unit = FusionConvAttention(
            in_channels=dim, out_channels=dim, reduction=reduction_ratio)
    ...
    def forward(self, x, relative_pos_enc=None):
        # 使用 FusionConvAttention 处理 x
        x = self.global_unit(x)
...
"""  
class HybridTokenMixer(nn.Module):# 只有IDConv2d 
    def __init__(self,
                 dim,
                 kernel_size=3,
                 num_groups=2,
        ...
        self.local_unit = DynamicConv2d(
            dim=dim, kernel_size=kernel_size, num_groups=num_groups)
...
class TransXNet(nn.Module):
    ...
    arch_settings = {
        **dict.fromkeys(['t', 'tiny', 'T'],
                        {'layers': [1, 1, 1, 1],#每个阶段的层数配置
                         'embed_dims': [48, 96, 224, 448],#每个阶段的嵌入维度
                         'kernel_size': [7, 7, 7, 7],#每层使用的卷积核大小
                         'num_groups': [2, 2, 2, 2],#每层的组数，通常用于分组卷积
                         'sr_ratio': [8, 4, 2, 1],#不同阶段的空间注意力比率
                         'num_heads': [1, 2, 4, 8],#每层的多头注意力头数
                         'mlp_ratios': [4, 4, 4, 4],#MLP 层的扩展比率
                         'layer_scale_init_value': 1e-5, }),#用于层缩放的初始值
    ...
    def __init__(self,
        ...
        mlp_ratios = arch['mlp_ratios'] if 'mlp_ratios' in arch else [4, 4, 4, 4]#从 arch 字典中提取 mlp_ratios，如果不存在，则将其初始化为 [4, 4, 4, 4]。这个变量通常表示多层感知器（MLP）的比率，影响隐藏层的大小
        ...
        for i in range(4):#启动一个循环，循环四次（通常与模型层数有关）。
        ...
        if self.fork_feat:#检查 fork_feat 属性，用于决定是否需要为每个输出添加规范化层。
            ...
            self.out_indices = [0, 2, 4, 6]#定义 self.out_indices，表示要输出的层的索引，通常用于选择中间层的输出。
    ...
    def init_weights(self, pretrained=None):#加载预训练模型的权重，确保模型在训练开始时具有合适的参数。
        ...
        if self.init_cfg is None and pretrained is None:
            ...
            print('unexpected_keys: ', unexpected_keys)#负责加载预训练模型的权重，确保模型在训练开始时具有合适的参数。
        ...
        for idx, layer in enumerate(self.network):
            if idx in [0, 2, 4, 6]:
                ...
                for blk in layer:
                    ...
                    print(f"Layer {idx} Block output shape: {x.shape}")  # 打印每个Block的输出尺寸
                pos_idx += 1
            else:
                # 直接通过网络层
                x = layer(x)
                print(f"Layer {idx} output shape: {x.shape}")  # 打印每层的输出尺寸
...
# 实例化模型
model = TransXNet(image_size=28, arch='base', num_classes=3)

# 创建一个随机输入图像，形状为 [B, C, H, W]，假设批大小 B = 1
...
# 获取每个阶段的输出尺寸
# 如果你启用了 `fork_feat`，输出是从多个阶段返回的特征
if model.fork_feat:
    for idx, stage_output in enumerate(output):
        print(f"Stage {idx + 1} output size: {stage_output.shape}")
else:

...
def whole_face_block_coordinates():
    ...
    m, n = df.shape
    base_data_src = r'C:\Users\24291\Desktop\CloFormer-main\dataset\data_MT\combined_datasets_whole'
    total_emotion = 0
    image_size_u_v = 224
...
def crop_optical_flow_block():
    # 其他部分保持不变
    whole_optical_flow_path = r'C:\Users\24291\Desktop\CloFormer-main\dataset\data_MT\STSNet_whole_norm_u_v_os'
...
def main(config):
    ...
    if (config.train):
        if not path.exists('../ourmodel_threedatasets_weightsDLAb'):
            os.mkdir('../ourmodel_threedatasets_weightsDLAb')#检查是否存在保存模型权重的文件夹，如果不存在则创建。
    ...
    main_path = r'C:\Users\24291\Desktop\CloFormer-main\dataset\data_MT\three_norm_u_v_os'
    ...
    for n_subName in subName:#遍历主数据集下的每个子文件夹
        ...
        weight_path = 'ourmodel_threedatasets_weightsDLAb' + '/' + n_subName + '.pth'
        ...
        for epoch in range(1, epochs + 1):#对每个epoch进行迭代。
            ...
            if best_accuracy_for_each_subject <= val_acc:
                ...
                if val_acc == 1.0:  # 如果测试准确率为100%
                    weight_path_subject = f'ourmodel_threedatasets_weightsDLAb/{n_subName}_best_accuracy.pth'
